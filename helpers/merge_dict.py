...
def export_to_json(eng_to_flang):

    dump_dict(eng_to_flang, 'types_es_es_es.json')    


def main():

    with open('types_es_es.json') as f:
        data = json.load(f)

    with open('filter_types.json') as f1:
	data.update(json.load(f1))

    with open('temporal_types.json') as f2:
	data.update({'ENTITY_temporal':json.load(f2)})

    export_to_json(data)


def get_vals(indexes, record):
    ll = []
...
def table_to_dict(table):
    ...
    for record in table:
	gid = record['gid']
	eng_titles = get_vals(['eng1', 'eng2'], record)
	spa_titles = get_vals(['spa1', 'spa2', 'spa3'], record)
	
	body = dict_obj.setdefault(gid, {})
  	for title in eng_titles:
	    if title in body:
		body[title] = list(set().union(spa_titles,body[title]))
	    else:
	        body[title] = spa_titles
    return dict_obj
